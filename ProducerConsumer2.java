...
// each consumer has its own queue, and the producers can produce in either queue (and choose the shortest one)
public class ProducerConsumer2 {
    ...
    // Constructor
    public ProducerConsumer2(int max, int numProducers, int numConsumers, int numProduceItems) {
    	this.max = max;
    	ProducerConsumer2.numProducers = numProducers; 
    	ProducerConsumer2.numConsumers = numConsumers; 
    ...
    // synchronized method to produce time stamps to queue
    public synchronized void produceItem(LocalDateTime timestamp, String producerName){
    	...
    	if (queue.size() > queue2.size()){
    		...
    		// the number of messages that each producer should produce
    		queue2.add(timestamp); // add number of time stamp to queue
    	}
    	notifyAll();
	}
    ...
    // method to run tests of the ProducerConsumer2 class constructor
	...
	public static void main(String[] args) {
		...
		ProducerConsumer2 buffer = new ProducerConsumer2(10,2,2,10);
